...
export const ViewReceiptModal: React.FC<ViewReceiptModalProps> = ({
  ...
  const linkedInvoice = receipt.invoiceNo
    ? invoices.find(
        (inv) => inv.invoiceNo === receipt.invoiceNo || inv.id === receipt.invoiceNo
      )
    : null;
  ...
  const disclaimer =
    'This is a system-generated Payment Receipt from Thulir Design & QS Services FZE. It is valid without a physical signature or company stamp.';
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-[220mm] shadow-2xl p-4 relative my-6 text-white print-hide">
        ...
        <div className="flex items-center justify-between pb-3 border-b border-zinc-800">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-xs text-zinc-300 font-mono font-bold whitespace-nowrap">
              {receipt.receiptNo}
            </span>
          </div>
          <div className="flex items-center space-x-2">
        ...
        <div className="a4-print-area rounded-lg border border-zinc-200 shadow-lg mt-3 text-zinc-900 font-sans text-[11px] p-7 space-y-4">
          ...
          <div className="flex justify-between items-start border-b-2 border-zinc-300 pb-4">
            <div className="flex-1">
              ...
              <p className="text-[9px] text-zinc-500">
                {companySettings.email} • Tax ID: {companySettings.taxId}
              </p>
            </div>
            <div className="text-right ml-4 shrink-0">
              ...
              <p className="font-mono text-sm font-extrabold text-emerald-700 mt-0.5 whitespace-nowrap">
                #{receipt.receiptNo}
              </p>
              <div className="mt-1.5 text-[9px] text-zinc-500">
                <p>
                  <span className="font-bold">Payment Date:</span> {receipt.paymentDate}
                </p>
                <p>
          ...
          <div className="grid grid-cols-2 gap-4 bg-slate-50 p-3 rounded-lg border border-slate-200">
            <div>
              <p className="text-[9px] font-bold uppercase text-slate-400 tracking-wider">
                {receipt.type === 'incoming' ? 'Received From:' : 'Paid To:'}
              </p>
            ...
            <div>
              ...
              <div className="mt-0.5 space-y-0.5 text-slate-700 text-[10px]">
                <p>
                  ...
                  {paymentModeLabel[receipt.paymentMode] || receipt.paymentMode}
                </p>
                <p>
                  <span className="font-semibold">Reference:</span>{' '}
                  <span className="font-mono">{receipt.referenceNo}</span>
                </p>
                <p>
                  <span className="font-semibold">Type:</span>{' '}
                  <span className="uppercase">{receipt.type}</span>
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-50 border-2 border-slate-300 rounded-lg p-4">
            <div className="flex justify-between items-center">
              <span className="font-bold text-slate-700 text-sm">
                Amount {receipt.type === 'incoming' ? 'Received' : 'Paid'}:
              </span>
              <span
                className={`font-mono font-black text-xl ${
                  receipt.type === 'incoming' ? 'text-emerald-700' : 'text-rose-700'
                }`}
              >
              ...
              <>
                <div className="flex justify-between items-center text-[11px] mt-2.5 pt-2.5 border-t border-slate-200">
                  ...
                  <span className="font-mono font-bold text-slate-700">
                    {formatCurrency(invoiceTotal, companySettings)}
                  </span>
                </div>
                <div className="flex justify-between items-center text-[11px] mt-1">
                  <span className="text-slate-500">Total Paid on Invoice:</span>
                  <span className="font-mono font-bold text-emerald-700">
                    {formatCurrency(linkedInvoice.paidAmount || 0, companySettings)}
                  </span>
                </div>
                <div className="flex justify-between items-center text-sm font-bold mt-1.5 pt-1.5 border-t-2 border-slate-300">
                  <span className="text-slate-800">Balance Due:</span>
                  <span
                    className={`font-mono text-base ${
                      balanceAfterPayment > 0 ? 'text-amber-700' : 'text-emerald-700'
                    }`}
                  >
          ...
          {hasBankDetails && (
            <div className="bg-slate-50 border border-slate-200 rounded-lg p-3">
              <p className="text-[9px] font-bold uppercase text-slate-500 tracking-wider mb-1.5">
          ...
          {/* Notes */}
          {receipt.notes && (
            <div className="border-t border-slate-200 pt-2.5 text-[10px] text-slate-500">
